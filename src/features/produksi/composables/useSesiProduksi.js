 ...
 * =================================================
 * Kontrak diikat ke produksi/RANCANGAN-API.md §D (pracindo_backend_v1).
 * Tidak ada endpoint yang dikarang di sini — kalau sebuah path tidak ada
 * di §D, dia tidak boleh muncul di file ini.
 *
 * PAGINASI (§D "Aturan rumah"):
...
import * as fx from '../_fixtures.dev'   // TODO: hapus saat Fase 2 backend live

const PAKAI_FIXTURE = true

export const STATUS_SESI = ['DRAFT', 'BERJALAN', 'SELESAI', 'GAGAL', 'BATAL']
export const JENIS_SESI = ['PRODUKSI', 'RND']

/** Status yang boleh dibatalkan — §D: batalkan/ hanya DRAFT. */
...
export function useSesiProduksi() {
  ...
  // =========================================================
  // SESI — §D "Sesi — jalur produksi rutin"
  // =========================================================
  ...
  const muatSesi = async (params = {}) => {
    ...
    galat.value = ''
    try {
      const { data } = PAKAI_FIXTURE
        ? await fx.fxSesiList(params)
        : await api.get('produksi/sesi/', { params })
  ...
  const muatDetail = async (id) => {
    ...
    try {
      const { data } = PAKAI_FIXTURE
        ? await fx.fxSesiDetail(id)
        : await api.get(`produksi/sesi/${id}/`)
      sesi.value = data
    } catch (err) {
   ...
   * Dua endpoint berbeda di §D, bukan satu endpoint dengan flag.
   */
  const buatSesi = async (jenis, payload) => {
    ...
    try {
      const { data } = PAKAI_FIXTURE
        ? await fx.fxSesiBuat({ ...payload, jenis_sesi: jenis })
  ...
  const jalankanAksi = async (id, aksi, payload = {}) => {
    ...
    try {
      const { data } = PAKAI_FIXTURE
        ? await fx.fxSesiAksi(id, aksi, payload)
        : await api.post(`produksi/sesi/${id}/${aksi}/`, payload)
      sesi.value = data
      return { success: true, data }
    } catch (err) {
  ...
  const muatPratinjauKerugian = async (id) => {
    ...
    try {
      const { data } = PAKAI_FIXTURE
        ? await fx.fxPratinjauKerugian(id)
        : await api.get(`produksi/sesi/${id}/pratinjau-kerugian/`)
  ...
  const muatBanding = async (ids = []) => {
    ...
    try {
      const { data } = PAKAI_FIXTURE
        ? await fx.fxBanding(ids)
        : await api.get('produksi/sesi/banding/', { params: { ids: ids.join(',') } })
      banding.value = data
    } catch (err) {
  ...
  const muatResep = async (params = {}) => {
    ...
    try {
      const { data } = PAKAI_FIXTURE
        ? await fx.fxResepList(params)
        : await api.get('produksi/resep/', { params })
  ...
  const muatKapasitas = async ({ grup, produk, tanggal }) => {
    ...
    try {
      const { data } = PAKAI_FIXTURE
        ? await fx.fxKapasitas({ grup, produk, tanggal })
        : await api.get('produksi/kapasitas/', { params: { grup, produk, tanggal } })
  ...
  const muatAcuan = async () => {
    try {
      const [g, e, p] = PAKAI_FIXTURE
        ? await Promise.all([fx.fxGrupBahan(), fx.fxEntitas(), fx.fxProduk()])
        : await Promise.all([
