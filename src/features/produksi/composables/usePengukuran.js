 ...
 * (§D: PUT/PATCH/DELETE -> 405, pola RiwayatAkses.save()).
 *
...
export function usePengukuran() {
  ...
  const muatKatalog = async () => {
    try {
      const { data } = PAKAI_FIXTURE
        ? await fx.fxJenisPengukuran()
        : await api.get('produksi/jenis-pengukuran/')
  ...
  const muatPengukuran = async (sesiId) => {
    ...
    try {
      const { data } = PAKAI_FIXTURE
        ? await fx.fxPengukuranList(sesiId)
        : await api.get(`produksi/sesi/${sesiId}/pengukuran/`)
  ...
  const muatCatatan = async (sesiId) => {
    ...
    try {
      const { data } = PAKAI_FIXTURE
        ? await fx.fxCatatanList(sesiId)
        : await api.get(`produksi/sesi/${sesiId}/catatan/`)
  ...
  const simpanPengukuran = async (sesiId, payload) => {
    ...
    try {
      const { data } = PAKAI_FIXTURE
        ? await fx.fxPengukuranBuat(sesiId, payload)
        : await api.post(`produksi/sesi/${sesiId}/pengukuran/`, payload)
  ...
  const simpanCatatan = async (sesiId, payload) => {
    ...
    try {
      const { data } = PAKAI_FIXTURE
        ? await fx.fxCatatanBuat(sesiId, payload)
        : await api.post(`produksi/sesi/${sesiId}/catatan/`, payload)
